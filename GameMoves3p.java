...
public class GameMoves3p extends JPanel implements KeyListener, ActionListener,MouseListener{
    ...
    public GameMoves3p() {
        ...
        current_player=0;
        la = new Layout3p(80,50);
        p = new Build_Player3p(la.height,la.width);
        dice=0;
        flag=0;
        roll=0;
        kill=0;
                ...
                if(value!=-1) {
                    ...
                    if((hou%13)!=0&&(hou%13)!=8&&hou<51)
                    {
                        for(int i=0;i<3;i++) {
                            if(i!=current_player) {
                                for(int j=0;j<3;j++) {
                                    int tem1= Path3p.ax[current_player][p.pl[current_player].pa[value].current],
                                            tem2= Path3p.ay[current_player][p.pl[current_player].pa[value].current];
